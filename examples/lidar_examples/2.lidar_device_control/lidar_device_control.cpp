...
void printfPropertyList(std::shared_ptr<ob::Device> device, const std::vector<OBPropertyItem> &propertyList) {
    ...
    for(size_t i = 0; i < propertyList.size(); i++) {
        auto        propertyItem = propertyList[i];
        std::string strRange     = "";

        OBIntPropertyRange   intRange;
        OBFloatPropertyRange floatRange;
        switch(propertyItem.type) {
        case OB_BOOL_PROPERTY:
            strRange = "Bool value(min:0, max:1, step:1)";
            break;
        case OB_INT_PROPERTY: {
            if(propertyItem.permission & OB_PERMISSION_READ) {
                try {
                    intRange = device->getIntPropertyRange(propertyItem.id);
                    strRange = "Int value(min:" + std::to_string(intRange.min) + ", max:" + std::to_string(intRange.max)
                               + ", step:" + std::to_string(intRange.step) + ")";
                }
                catch(...) {
        ...
        std::cout.width(2);
        std::cout << i << ". ";
        std::cout << propertyItem.name << "(" << (int)propertyItem.id << ")";
        std::cout << ", permission=" << permissionTypeToString(propertyItem.permission) << ", range=" << strRange << std::endl;
    }
    std::cout << "------------------------------------------------------------------------\n";
}
